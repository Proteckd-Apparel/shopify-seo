...
import {
  applyMetaDescToOne,
  bulkApplyMetaDescTemplate,
  previewMetaDescForResource,
  previewMetaDescTemplate,
  restoreLastMetaDescRun,
  saveMetaDescTemplate,
  searchResourcesForDescPicker,
  type BulkResult,
  type DescPreviewSample,
...
export function MetaDescTemplateMode({
  initialTemplates,
}: {
  ...
  const [sample, setSample] = useState<DescPreviewSample | null>(null);
  ...
  function save() {
    setMsg(null);
    start(async () => {
      const r = await saveMetaDescTemplate(scope, tpl);
      setMsg(r.message);
    });
  }
  ...
  function runPreview(index = 0) {
    setMsg(null);
    start(async () => {
      const r = await previewMetaDescTemplate(scope, tpl, index);
  ...
  function previewSpecific(resourceId: string) {
    setMsg(null);
    start(async () => {
      const r = await previewMetaDescForResource(tpl, resourceId);
      if (r.ok && r.sample) setSample(r.sample);
      else setMsg(r.message ?? "Preview failed");
    });
  }
  ...
  function applyOne() {
    ...
    setMsg(null);
    start(async () => {
      const r = await applyMetaDescToOne(scope, tpl, sample.resourceId);
  ...
  function restore() {
    if (
      !confirm(
        `Restore meta descriptions for ${scope} from the last 60 minutes?\n\nReverts every ${scope.slice(0, -1)} the optimizer touched recently.`,
      )
    )
      return;
    setMsg(null);
    start(async () => {
      const r = await restoreLastMetaDescRun(scope, 60);
      setMsg((r.ok ? "✅ " : "❌ ") + r.message);
    });
  }
  ...
  function runBulk() {
    if (
      !confirm(
        `Apply this template to ${overwrite ? "ALL" : "missing"} meta descriptions in ${scope}?\n\nThis writes to Shopify.`,
      )
    )
      return;
    setBulk(null);
    start(async () => {
      const r = await bulkApplyMetaDescTemplate(
        scope,
        tpl,
        overwrite,
        updateScope,
  ...
  return (
    <div className="max-w-4xl space-y-4">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-900">
        The meta description appears below the page title in Google search
        results. Visitors on your site cannot see it. When overwrite is OFF,
        only resources with missing meta descriptions are updated.
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg">
        ...
        <div className="p-5 space-y-4">
          ...
          <TemplateSettings
            value={tpl}
            onChange={setTpl}
            maxCharsHint="Google truncates around 160."
          />
        </div>
      </div>
            ...
            <div className="md:col-span-2 p-5 space-y-3 border-r border-slate-100">
              <div className="bg-amber-50 border border-amber-200 rounded p-3">
                <div className="text-[10px] uppercase font-semibold text-amber-700">
                  Current Meta Description ({sample.currentValue?.length ?? 0})
                </div>
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded p-3">
                <div className="text-[10px] uppercase font-semibold text-emerald-700 flex items-center gap-2">
                  New Meta Description ({sample.newValue.length})
                  {tpl.maxChars > 0 &&
      ...
      <div className="sticky bottom-4 bg-white border border-slate-200 rounded-lg p-3 flex flex-wrap items-center gap-2 shadow-lg">
        ...
        <button
          ...
          title="Revert any meta description changes made to this scope in the last 60 minutes"
        >
...
function Picker({
  ...
  function doSearch(value: string) {
    ...
    start(async () =>
      setResults(await searchResourcesForDescPicker(scope, value)),
    );
  }
  return (
