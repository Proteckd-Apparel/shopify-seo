...
`
  ...
  // Whitespace control: group.user_agent renders as "User-agent: *" with NO
  // trailing newline, and rule renders as "Disallow: /foo\n" WITH a trailing
  // newline. So we use {{- group.user_agent }} (left-strip only) so the
  // newline AFTER the tag in source survives, separating it from the first
  // rule. Same logic applied between groups so each block is on its own line.
...
${list
  .filter((r) => r.type !== "sitemap")
  .map((r) => `${capitalize(r.type)}: ${escapeLiquid(r.value)}`)
  .join("\n")}
{% endif -%}`,
  )
  .join("\n")}
